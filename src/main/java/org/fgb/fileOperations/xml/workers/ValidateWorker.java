 ...
 *
 */
public class ValidateWorker extends SwingWorker<String, String> {
	/**
	 * The name of this class.
	 */
	private static final String _className = ValidateWorker.class.getName();
	/**
	 * JDK logger.
	 */
	...
	public ValidateWorker(final JComponent panel, List<File> files) {
		System.out.println(_className + ".ValidateWorker(JPanel j): panel.getName() = " + panel.getName());
		this.resultsPanel = panel;
		Component c;
		if ((c = SwingUtils.findComponentByName(panel, "ResultsTextArea")) != null) {
			this.resultsTextArea = (JTextArea) c;
			System.out.println(_className + ".ValidateWorker(): c.getName() = " + c.getName());
		}
